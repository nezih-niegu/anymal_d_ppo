...
class Env:
    ...
    def step(self, raw_action, render=False):
        ...
        action_cost = -0.003 * float(np.sum(bounded**2))

        # Discourage jerky changes between consecutive commands.
        smooth_cost = -0.004 * float(np.sum((bounded - self.last_action) ** 2))

        # Very mild penalty on joint velocity — bounds energy without
        # suppressing the swing phase.
        joint_vel_cost = -0.0003 * float(np.sum(self.data.qvel[6:] ** 2))
...
def train_or_sweep(
    ...
    _ckpt = "pretrained_models/anymal_d/brave-forest-1_5931_Reward-1805.2_policy.pt"
    if os.path.exists(_ckpt):
        policy = torch.load(_ckpt, map_location="cpu", weights_only=False)
        print(f"Resumed from checkpoint: {_ckpt}")
