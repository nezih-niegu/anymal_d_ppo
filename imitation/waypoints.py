...
class WaypointDemoDataset(Dataset):
    ...
    def __init__(
        ...
        print(
            f"  obs={self.obs.shape}  actions={self.actions.shape}  waypoints={self.waypoints.shape}"
        )

    @staticmethod
    def _build_waypoints(
        xyz: np.ndarray, ep_ids: np.ndarray, stride: int
    ) -> np.ndarray:
...
def make_waypoint_dataloader(npz_path, batch_size=512, normalize=True, num_workers=2):
