...
def main():
    ...
    metrics = {
        ...
        "Fwd velocity (m/s)": [
            ppo["summary"]["forward_velocity_mean"],
            flow["summary"]["forward_velocity_mean"],
        ],
        "Fall rate": [
            f"{ppo['summary']['fall_rate']*100:.0f}%",
            f"{flow['summary']['fall_rate']*100:.0f}%",
        ],
        "Consistency (std)": [smoothness(ppo_eps), smoothness(flow_eps)],
        "Ep length mean": [
            ppo["summary"]["episode_length_mean"],
            flow["summary"]["episode_length_mean"],
        ],
    }
    ...
    fig.suptitle(
        "PPO vs Flow Matching — Policy Comparison", fontsize=14, fontweight="bold"
    )
    ...
    ax.axhline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f"PPO mean={np.mean(ppo_rewards):.0f}",
    )
    ax.axhline(
        np.mean(flow_rewards),
        color="#DD8452",
        linestyle="--",
        linewidth=1.5,
        label=f"FM mean={np.mean(flow_rewards):.0f}",
    )
